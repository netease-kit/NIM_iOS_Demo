...
public protocol QChatServerProviderDelegate: Any {
    func callBack()
}
...
public class QChatServerProvider: NSObject,NIMQChatServerManagerDelegate {
    ...
    //创建服务器
    public func createServer(param:CreateServerParam,_ completion:@escaping (NSError?,CreateServerResult?) ->()){
    ...
    //查询服务器信息
    public func getServers(param:QChatGetServersParam,_ completion:@escaping (NSError?,QChatGetServersResult?)->()){
        NIMSDK.shared().qchatServerManager.getServers(param.toIMParam()) { error, getServersResult in
            completion(error as NSError?,QChatGetServersResult(serversResult: getServersResult))
        }
    }
    
    //查询服务器列表
    public func getServerCount(param:GetServersByPageParam,_ completion:@escaping  (NSError?,GetServersByPageResult?) -> ()) {
    ...
    //申请加入服务器
    public func applyServerJoin(param:QChatApplyServerJoinParam,_ completion:@escaping (NSError?)->()){
        NIMSDK.shared().qchatServerManager.applyServerJoin(param.toIMParam()) { error in
            completion(error as NSError?)
        }
    }
    
    //查询服务器内成员信息
    public func getServerMembers(param:QChatGetServerMembersParam,_ completion:@escaping (NSError?,QChatGetServerMembersResult?)->()){
    ...
    //分页查询服务器成员信息
    public func getServerMembersByPage(param:QChatGetServerMembersByPageParam,_ completion:@escaping (NSError?,QChatGetServerMembersResult?)->()){
        
        NIMSDK.shared().qchatServerManager.getServerMembers(byPage: param.toIMParam()) { error, result in
            completion(error as NSError?,QChatGetServerMembersResult(membersResult: result))
        }
    }
    
    //邀请服务器成员
    public func inviteMembersToServer(param:QChatInviteServerMembersParam,_ completion:@escaping (NSError?)->()){
        NIMSDK.shared().qchatServerManager.inviteServerMembers(param.toImParam()) { error in
            completion(error as NSError?)
        }
    }
    ...
    public func getServerMembers(_ param: GetServerMembersByPageParam,  _ completion: @escaping (Error?, [ServerMemeber]) -> Void ){        
        NIMSDK.shared().qchatServerManager.getServerMembers(byPage: param.toImParam()) { error, result in
    ...
    public func updateMyServerMember(_ param: UpdateMyMemberInfoParam, _ completion: @escaping (Error?, ServerMemeber) -> Void){
        NIMSDK.shared().qchatServerManager.updateMyMemberInfo(param.toImParam()) { error, member in
            completion(error, ServerMemeber(member))
        }
    }
    
    public func updateServerMember(_ param: UpdateServerMemberInfoParam, _ completion: @escaping (Error?, ServerMemeber) -> Void){
        NIMSDK.shared().qchatServerManager.updateServerMemberInfo(param.toImPara()) { error, member in
            completion(error, ServerMemeber(member))
        }
    }
    
    public func kickoutServerMembers(_ param: KickServerMembersParam, _ completion: @escaping (Error?) -> Void ){
