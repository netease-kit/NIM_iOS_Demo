//
//  QChatRoleProvider.swift
//  CoreKit-IM
//
//  Created by yu chen on 2022/2/9.
//
...
import UIKit
import AVFoundation
public protocol QChatRoleProviderDelegate: AnyObject {
    
}

public class QChatRoleProvider {
    ...
    public func createRole(_ param: ServerRoleParam, _ completion: @escaping (Error?, ServerRole) -> Void ){
    ...
    //    获取服务器身份组
    public func getRoles(_ param: GetServerRoleParam, _ completion: @escaping (Error?, [ServerRole]?, Set<NSNumber>?) -> Void) {
    ...
    public func updateServerRolePriorities(_ param: UpdateServerRolePrioritiesParam, _ completion: @escaping  (Error?) -> Void){
        
        NIMSDK.shared().qchatRoleManager.updateServerRolePriorities(param.toImParam()) { error, result in
            completion(error)
        }
    }
    
    public func deleteRoles(_ param: DeleteServerRoleParam, _ completion: @escaping (Error?) -> Void){
        NIMSDK.shared().qchatRoleManager.deleteServerRole(param.toIMParam()) { error in
            completion(error)
        }
    }
    
    public func updateRole(_ param: UpdateServerRoleParam, _ completion: @escaping (Error?, ServerRole) -> Void) {
    ...
    public func getServerRoleMembers(_ param: GetServerRoleMembersParam, _ completion: @escaping (Error?, [RoleMember]) -> Void){
    ...
    public func addRoleMember(_ param: AddServerRoleMemberParam, _ completion: @escaping (Error?, [String], [String]) -> Void){
    ...
    public func deleateRoleMember(_ param: RemoveServerRoleMemberParam, _ completion: @escaping (Error?, [String], [String]) -> Void){
        NIMSDK.shared().qchatRoleManager.removeServerRoleMember(param.toImParam()) { error, result in
            var success = [String]()
            var faileds = [String]()
    ...
    //添加身份组到某个频道下
    public func addChannelRole(param: AddChannelRoleParam, _ completion: @escaping (NSError?, ChannelRole?)->()) {
    ...
    //移除某个频道下的身份组
    public func removeChannelRole(param: RemoveChannelRoleParam, _ completion: @escaping (NSError?)->()) {
    ...
    //查询频道下身份组列表
    public func getChannelRoles(param: ChannelRoleParam ,_ completion: @escaping (NSError?, [ChannelRole]?)->()) {
    ...
    public func getExistingChannelRoles(param: GetExistingChannelRolesByServerRoleIdsParam ,_ completion: @escaping (NSError?, [ChannelRole]?)->()) {
        NIMSDK.shared().qchatRoleManager.getExistingChannelRoles(byServerRoleIds: param.toIMParam()) { error , result in
            guard let roleArray = result?.channelRoleArray else {
                completion(error as NSError?, nil)
                return;
            }
    ...
    //查询频道下成员的权限
    public func getMemberRoles(param: GetMemberRolesParam ,_ completion: @escaping (NSError?, [MemberRole]?)->()) {
    ...
    // 设置频道下身份组权限
    public func updateChannelRole(param: UpdateChannelRoleParam ,_ completion: @escaping (NSError?, ChannelRole?)->()) {
        NIMSDK.shared().qchatRoleManager.updateChannelRole(param.toIMParam()) { error , channelRole in
            completion(error as NSError?, ChannelRole(role: channelRole))
        }
    }
    
    //通过accid查询自定义身份组列表
    public func getServerRolesByAccId(param: GetServerRolesByAccIdParam ,_ completion: @escaping (Error?, [ServerRole]?)->()) {
        let imParam = param.toIMParam()
    ...
    //查询一批accids的自定义身份组列表
    public func getExistingServerRolesByAccids(param: QChatGetExistingAccidsInServerRoleParam ,_ completion: @escaping (NSError?, [String:[ServerRole]]?)->()){
        NIMSDK.shared().qchatRoleManager.getExistingAccids(inServerRole: param.toImParam()) { error, result in
    ...
    // 添加成员到频道
    public func addMemberRole(_ param: AddMemberRoleParam, _ completion: @escaping (NSError?, MemberRole?)->()) {
    ...
    // 移除某个频道下的成员
    public func removeMemberRole(param: RemoveMemberRoleParam, _ completion: @escaping (NSError?)->()) {
    ...
    // 设置某个频道下的成员的权限
    public func updateMemberRole(param: UpdateMemberRoleParam, _ completion: @escaping (NSError?, MemberRole?)->()) {
    ...
    public func getExistingMemberRoles(param: GetExistingAccidsOfMemberRolesParam ,_ completion: @escaping (NSError?, [MemberRole]?)->()) {
        print(#function + "⬆️accid:\(param.accids)")
        NIMSDK.shared().qchatRoleManager.getExistingAccids(ofMemberRoles: param.toIMParam()) { error, result in
    ...
    public func getExistingServerRoleMembersByAccids(_ param: GetExistingServerRoleMembersByAccidsParam, _ completion: @escaping (Error?, [String])->()){
        
        NIMSDK.shared().qchatRoleManager.getExistingServerRoleMembers(byAccids: param.toImParam()) { error, result in
            var accids = [String]()
