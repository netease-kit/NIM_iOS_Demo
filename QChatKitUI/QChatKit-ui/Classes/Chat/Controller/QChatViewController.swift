...
public class QChatViewController: NEBaseViewController, UINavigationControllerDelegate, QChatInputViewDelegate, QChatViewModelDelegate{
    ...
    private lazy var brokenNetworkView:QChatBrokenNetworkView = {
        let view = QChatBrokenNetworkView.init(frame: CGRect.init(x: 0, y: kNavigationHeight + KStatusBarHeight, width: kScreenWidth, height: 36))
        return view
    }()
    ...
    func didClickMessage(messgae:NIMQChatMessage) {
        if messgae.messageType == .image {
            ...
            if let imageUrl =  imageObject.url {
                let showController = QChatShowImageController(urls: [imageUrl], url: imageUrl)
