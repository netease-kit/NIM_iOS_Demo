...
- (instancetype)initWithCallerInfo:(NTESTeamMeetingCallerInfo *)info
{
    ...
    if (self)
    {
        [self setup:info.members teamId:info.teamId teamType:info.teamType];
           ...
           meeting:(NIMNetCallMeeting *)meeting
{
    if ([meeting.name isEqualToString:self.meeting.name])
    {
        ...
        if (indexPath)
        {
            ...
            [cell refreshWithRemoteView:nil];
        }
    }
}
...
- (void)onRemoteDisplayviewReady:(UIView *)displayView user:(NSString *)user {
    ...
    {
        NTESTeamMeetingCollectionViewCell *cell = (NTESTeamMeetingCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
        [cell refreshWithRemoteView:displayView];
    }
}
...
#pragma mark - Private
- (void)setup:(NSArray *)members teamId:(NSString *)teamId teamType:(NIMKitTeamType)teamType
{
    ...
    if (teamType == NIMKitTeamTypeSuper) {
        _team = [[NIMSDK sharedSDK].superTeamManager teamById:teamId];
        _session = [NIMSession session:_team.teamId type:NIMSessionTypeSuperTeam];
    } else {
        _team = [[NIMSDK sharedSDK].teamManager teamById:teamId];
        _session = [NIMSession session:_team.teamId type:NIMSessionTypeTeam];
    }
    _enableSpeaker = YES;
}
...
- (void)sendCallNotification
{
    ...
    [_notificationSender sendCallNotification:self.team roomName:self.meeting.name members:members];
}
