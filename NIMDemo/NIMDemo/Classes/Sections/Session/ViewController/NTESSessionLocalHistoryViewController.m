...
- (void)prepareMember{
    if (self.session.sessionType == NIMSessionTypeTeam) {
        ...
        [[NIMSDK sharedSDK].superTeamManager fetchTeamMembers:self.session.sessionId option:option completion:^(NSError * _Nullable error, NSArray<NIMTeamMember *> * _Nullable members) {
            NSMutableArray *data = [[NSMutableArray alloc] init];
            for (NIMTeamMember *member in members) {
                if(member.userId.length){
                    [data addObject:member.userId];
