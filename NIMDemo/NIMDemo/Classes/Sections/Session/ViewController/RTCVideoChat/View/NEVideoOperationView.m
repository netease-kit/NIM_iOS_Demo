...
- (void)setupUI {
    ...
    UIStackView *stackView = [[UIStackView alloc] initWithArrangedSubviews:@[self.switchAudio,self.microPhone,self.cameraBtn,self.hangupBtn]];
...
- (UIButton *)switchAudio {
    if (!_switchAudio) {
        _switchAudio = [UIButton buttonWithType:UIButtonTypeCustom];
        [_switchAudio setImage:[UIImage imageNamed:@"call_change_audio"] forState:UIControlStateNormal];
        [_switchAudio setImage:[UIImage imageNamed:@"call_change_audio"] forState:UIControlStateSelected];
    }
    return _switchAudio;
}
